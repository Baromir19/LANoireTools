...
* `uber_unpack.py` - unpacks the .uber file (ptM#). It is currently useless, it only shows approximate values ​​that are behind the pointer addresses, but they are not ordered and are far from the original DAE. Also some pointers point to other files (as far as I understand, only to DDS). In the future I will try to figure out how the game reads this file;
